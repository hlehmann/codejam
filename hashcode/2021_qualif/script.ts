...
type Intersection = {
  id: number;
  lights: IntersectionLight[];
  expectedOrders: number[];
  uniform: boolean;
  goodForFirstCar: boolean;
};
type State = {
...
const load = () => {
  ...
  const intersections: Intersection[] = range(I).map((id) => ({
    id,
    lights: [],
    cycleTime: 0,
    expectedOrders: [],
    uniform: true,
    goodForFirstCar: false,
  }));
  const streets: Street[] = getLines(S).map((s, id) => {
    const [from, to, name, time] = s.split(" ");
    return {
      id,
      from: intersections[parseInt(from)],
      to: intersections[parseInt(to)],
      name,
      time: parseInt(time),
      expectedOrder: -2,
      crosses: 0,
  ...
  const cars: Car[] = getLines(V).map((s, id) => ({
    id,
    streets: s
      .split(" ")
      .slice(1)
      .map((name) => indexedStreets.get(name)!),
    remainingStreets: [],
    arrival: 0,
    time: 0,
  }));
        ...
        if (street.to.uniform && street.cars.length && street.cars[0].arrival <= time && street.expectedOrder === -1) {
          ...
          while (intersection.expectedOrders.includes(expectedOrder)) {
            ...
            if (expectedOrder === 0 && street.to.lights.length === 1) {
              ...
              console.log(
                state.streets.find((street) => street.to === intersection && street.expectedOrder === expectedOrder)
              );
              throw new Error();
            }
          }
  ...
  const improveCrosses = (state: State, coef: number) => {
    state.streets.forEach((street) => {
      if (
        street.cars.length &&
        street.cars[0].arrival < D &&
        !street.to.lights.find((light) => light.street === street)
      ) {
        street.crosses = 1;
    ...
    state.intersections.forEach((intersection) => {
      ...
      // const temp = lights.slice().sort((a,b) => streets[a.streetId].loses - streets[b.streetId].loses)
      const temp = intersection.lights
        .slice()
        .filter((a) => a.street.crosses)
        .sort((a, b) => b.street.crosses - a.street.crosses);
      if (temp.length) {
  ...
  const improveCrossesLight = (state: State, coef: number) => {
    state.intersections.forEach((intersection) => {
      ...
      if (intersection.lights.length <= 1) return;
      // const temp = lights.slice().sort((a,b) => streets[a.streetId].loses - streets[b.streetId].loses)
      const temp = intersection.lights.slice().sort((a, b) => b.street.crosses - a.street.crosses);
      if (temp.length) {
        const max = temp[0].street.crosses;
        // if (max < 5 || temp[temp.length - 1].cars > max/2) return
        // console.log("")
