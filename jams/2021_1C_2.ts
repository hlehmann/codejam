...
import { loadStdin, loadSample, getParsedSplitedLine, runner, getParsedLine, getLine } from "../snippet/runner";
...
const test = () => {
  ...
  const getCombinaisonFirstNumber = (combinaison: Combinaison, from: bigint) => {
    ...
    if (numbersAfterJump) {
      ...
      const n = BigInt(range(totalNumbers).map(i => first + i).join(""));
      ...
      // ckeck next if equal
      let n = BigInt(range(totalNumbers).map(i => first + i).join(""));
      if (BigInt(n) > from) return n;
      first++;
      if (first > 10**size - numbersBeforeJump) return;
      n = BigInt(range(totalNumbers).map(i => first + i).join(""));
      return n;      
    }
  }
